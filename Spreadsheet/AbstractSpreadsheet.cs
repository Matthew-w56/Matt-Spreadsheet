...
namespace SS {
	...
	/// </list>   
	/// <para>
	///     Note that this is the same as the definition of valid variable from theFormula class assignment.
	/// </para>
	/// 
	/// <para>
	...
	///      as reported by the Evaluate method of the Formula class.  The value ofa Formula,
	...
	public abstract class AbstractSpreadsheet {
		...
		protected IEnumerable<String> GetCellsToRecalculate(ISet<String> names) {
			//Initialize lists to contains which cells have been seen so far.
			//'changed' acts as a depth-first list of cells, so that it will be traversed in the correct
			//order for recalculation
			//'visited' acts as a breadth-first list of cells, so that no cell is added to the list twice.
			LinkedList<String> changed = new LinkedList<String>();
			HashSet<String> visited = new HashSet<String>();

			//Each name in the list is a starting place for the recursive search
			foreach (String name in names) {
				//If that cell isn't already visited (possibly by an earlier token in 'names')
				if (!visited.Contains(name)) {
					//Begin the recursive search on this cell
					Visit(name, name, visited, changed);
				}
			}

			//Return the depth-first-ordered list of cells
			return changed;
		}
		...
		private void Visit(String start, String name, ISet<String> visited,
			...
			//Begin by adding the cell being visited to the breadth-first array
			visited.Add(name);

			//For every direct dependent that this cell has
			foreach (String n in GetDirectDependents(name)) {
				//Make sure that none of the cells are the first one that got checked
				//('start' being the cell passed in by GetCellsToRecalculate())
				if (n.Equals(start)) {
					throw new CircularException();
				}
				//If this cell hasn't been visited yet, visit it next
				else if (!visited.Contains(n)) {
					Visit(start, n, visited, changed);
				}
			}
			//Now that all dependents have been visited and finished,
			//add this cell to the depth-first-ordered array
